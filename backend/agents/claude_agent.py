...
class ChatClaude(BaseAgent):
    def __init__(self) -> None:
        super().__init__()
        self._llm: Optional[Any] = None

    def set_agent(self, model: str, api_key: str) -> "ChatClaude":
        try:
            self._llm = ChatAnthropic(model=model, anthropic_api_key=api_key, temperature=0)
            return self
        except Exception as e:
            raise Exception(f"Invalid API key or model: {e}")

    def bind_tools(self, tools: List[Callable]) -> "ChatClaude":
        if not self._llm:
