...
from backend.db.log import PostgresLogger
import numpy as np
import os
import pandas as pd
...
class DataConfig:
    ...
    def set_df(self, dataframe: Optional[pd.DataFrame] = None) -> None:
        if dataframe is not None:
            ...
            data_path = os.path.join(root_dir, "backend", "uploads", "df.csv")
...
class Config:
    ...
    analyser_config: Analyser_config = Analyser_config()
    visualization_config: Visualizer_config = Visualizer_config()
    llm_config: LLMConfig = LLMConfig()
    tool_config: ToolConfig = ToolConfig()
    prompt_config: Promptconfig = Promptconfig()
    data_config: DataConfig = DataConfig()
    logger: Optional[PostgresLogger] = None