...
class LoggingService:
    """Simple PostgreSQL-based logging service for preprocessing tool usage."""
    ...
    def log_tool_usage(self, tool_name: str) -> int:
        ...
        insert_query = "INSERT INTO tool_usage_log (timestamp, tool_name) VALUES (%s, %s) RETURNING id"
    ...
    def get_log_by_id(self, log_id: int) -> Optional[Dict]:
        ...
        """
        query = "SELECT id, timestamp, tool_name FROM tool_usage_log WHERE id = %s"
    ...
    def get_tool_usage_sequence(self) -> List[str]:
        ...
        try:
            ...
            cursor.execute(query)
            rows = cursor.fetchall()
            result = [row['tool_name'] for row in rows]
            cursor.close()
            return result
        except psycopg2.Error as e:
    ...
    def get_all_batches(self) -> List[int]:
        ...
        try:
            ...
            cursor.execute(query)
            rows = cursor.fetchall()
            result = [row['batch_id'] for row in rows]
            cursor.close()
            return result
        except psycopg2.Error as e:
